#seed in 1234 2345 3456 4567 5678;
...
for seed in 1234 2345 3456; do
    for contrast in "true" "false"; do
        for thecurrentfeature in "pcen" "mfcc" "logmel" "pcen@mfcc" "mel" "logmel@mfcc" "logmel@delta_mfcc" "pcen@delta_mfcc"; do
            echo "----Running seed number ${seed}, negative_train_contrast: ${contrast}, feature: ${thecurrentfeature} ----"
            #python3 train.py features.feature_types=${thecurrentfeature} train_param.negative_train_contrast=${contrast} seed=${seed} exp_name="${thecurrentfeature}-neg_contrast${contrast}-seed${seed}"
        done
    done
done